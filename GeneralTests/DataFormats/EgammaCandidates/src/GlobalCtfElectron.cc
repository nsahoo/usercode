...
GlobalCtfElectron::GlobalCtfElectron(const SuperClusterRef scl, const TrackRef gsft,
...
float GlobalCtfElectron::ecalEta(float EtaParticle , float Zvertex, float plane_Radius)
{
  if (EtaParticle!= 0.)
    {
      float Theta = 0.0  ;
...
float GlobalCtfElectron::ecalPhi(float PtParticle, float EtaParticle, float PhiParticle, int ChargeParticle, float Rstart)
{
  //Magnetic field
...
//void GlobalCtfElectron::correctElectronEnergyScale(const PElectronEnergyCorrector *thecorr) {
void GlobalCtfElectron::correctElectronEnergyScale(const float newEnergy) {
...
//void GlobalCtfElectron::correctElectronFourMomentum(const PElectronMomentumCorrector *thecorr) {
void GlobalCtfElectron::correctElectronFourMomentum(const math::XYZTLorentzVectorD & momentum,float & enErr, float & tmErr) {
...
//void GlobalCtfElectron::classifyElectron(const PElectronClassification *theclassifier) {
//   electronClass_ = theclassifier->getClass();
void GlobalCtfElectron::classifyElectron(const int myclass)
{
  electronClass_ = myclass;
}

bool GlobalCtfElectron::overlap( const Candidate & c ) const {
...
GlobalCtfElectron * GlobalCtfElectron::clone() const { 
  return new GlobalCtfElectron( * this ); 
}
