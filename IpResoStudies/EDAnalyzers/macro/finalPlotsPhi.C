...
void finalPlotsPhi(int type,bool skip3rd=false){
  //-----
  ...
  TString fileName2="./finalPlots1GeV/histos"+counter+".root";
  ...
  //TH1F* hMC2   = (TH1F*) f2->Get("plotReso");
  ...
  double phi;
  double values[18];
  TH1F* tmpH = new TH1F("tmpH","tmpH",200,-M_PI,M_PI);
  for(int i=-9,j=0; i < 9; i++,j++){
    phi = 2*M_PI/18.*i;
    values[j]=phi;
    cout << "i,phi: " << i << " , " << values[j] << endl;   
    tmpH->Fill(phi,140.);
  }
  tmpH->SetLineColor(2);
  tmpH->SetLineWidth(2);
