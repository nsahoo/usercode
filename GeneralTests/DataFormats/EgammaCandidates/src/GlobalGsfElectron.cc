...
GlobalGsfElectron::GlobalGsfElectron(const SuperClusterRef scl, const GsfTrackRef gsft,
...
float GlobalGsfElectron::ecalEta(float EtaParticle , float Zvertex, float plane_Radius)
{
  if (EtaParticle!= 0.)
    {
      float Theta = 0.0  ;
...
float GlobalGsfElectron::ecalPhi(float PtParticle, float EtaParticle, float PhiParticle, int ChargeParticle, float Rstart)
{
  //Magnetic field
...
//void GlobalGsfElectron::correctElectronEnergyScale(const PElectronEnergyCorrector *thecorr) {
void GlobalGsfElectron::correctElectronEnergyScale(const float newEnergy) {
...
//void GlobalGsfElectron::correctElectronFourMomentum(const PElectronMomentumCorrector *thecorr) {
void GlobalGsfElectron::correctElectronFourMomentum(const math::XYZTLorentzVectorD & momentum,float & enErr, float & tmErr) {
...
//void GlobalGsfElectron::classifyElectron(const PElectronClassification *theclassifier) {
//   electronClass_ = theclassifier->getClass();
void GlobalGsfElectron::classifyElectron(const int myclass)
{
  electronClass_ = myclass;
}

bool GlobalGsfElectron::overlap( const Candidate & c ) const {
...
GlobalGsfElectron * GlobalGsfElectron::clone() const { 
  return new GlobalGsfElectron( * this ); 
}
